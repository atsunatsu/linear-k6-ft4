...
typedef struct __attribute__((__packed__)) {
	...
	struct __attribute__((__packed__)) {
		...
		uint8_t  RetuneMode;
		uint8_t  Padding;
		uint32_t CurrentTxFrequencyHz;
		uint32_t LastAppliedTxFrequencyHz;
		uint32_t PendingTxFrequencyHz;
		uint16_t FramesSent;
...
#ifdef ENABLE_FT4_CLEAN_TX
static void SendFt4Reply(uint16_t reply_id, FT4TX_Status_t status)
{
	REPLY_0700_t Reply;
	FT4TX_StateSnapshot_t snapshot;

	FT4TX_GetState(&snapshot);

	Reply.Header.ID = reply_id;
	Reply.Header.Size = sizeof(Reply.Data);
	Reply.Data.Status = status;
	Reply.Data.Active = snapshot.active;
	Reply.Data.RetuneMode = snapshot.retune_mode;
	Reply.Data.Padding = 0;
	Reply.Data.CurrentTxFrequencyHz = snapshot.current_tx_frequency_hz;
	Reply.Data.LastAppliedTxFrequencyHz = snapshot.last_applied_tx_frequency_hz;
	Reply.Data.PendingTxFrequencyHz = snapshot.pending_tx_frequency_hz;
	Reply.Data.FramesSent = snapshot.frames_sent;
	Reply.Data.FrameSequence = snapshot.frame_sequence;

	SendReply(&Reply, sizeof(Reply));
}

static void CMD_0701_StartFt4Tx(const uint8_t *pBuffer)
{
	const CMD_0701_t *pCmd = (const CMD_0701_t *)pBuffer;
	FT4TX_Status_t status;

	if (pCmd->Timestamp != Timestamp)
		status = FT4TX_STATUS_TX_NOT_READY;
	else
		status = FT4TX_Start(pCmd->Payload, pCmd->PayloadLength, pCmd->TxFrequencyHz);

	SendFt4Reply(0x0702, status);
}

static void CMD_0703_UpdateFt4Frequency(const uint8_t *pBuffer)
{
	const CMD_0703_t *pCmd = (const CMD_0703_t *)pBuffer;
	FT4TX_Status_t status;

	if (pCmd->Timestamp != Timestamp)
		status = FT4TX_STATUS_TX_NOT_READY;
	else
		status = FT4TX_UpdateFrequency(pCmd->TxFrequencyHz);

	SendFt4Reply(0x0704, status);
}

static void CMD_0705_StopFt4Tx(const uint8_t *pBuffer)
{
	const CMD_0705_t *pCmd = (const CMD_0705_t *)pBuffer;
	FT4TX_Status_t status;
...
#ifdef ENABLE_FT4_CLEAN_TX
		case 0x0701:
			CMD_0701_StartFt4Tx(UART_Command.Buffer);
			break;

		case 0x0703:
			CMD_0703_UpdateFt4Frequency(UART_Command.Buffer);
			break;

		case 0x0705:
			CMD_0705_StopFt4Tx(UART_Command.Buffer);
			break;
#endif
	}
}
