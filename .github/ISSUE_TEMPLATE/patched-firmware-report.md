---
name: patched 固件测试报告
about: 提交 patched 固件 + patched DigiManager 的实机测试结果
title: "[patched-ft4-test] "
labels: ["patched-ft4-test"]
assignees: []
---
...
- 数字模式入口是否仍存在：`yes / no`
- DigiManager 是否正常启动：`yes / no`

## 第一阶段：固定频点合规 FT4
- FT8 是否正常：`yes / no`
- FT4 是否进入 TX：`yes / no`
- 第二接收机 / 第二套 WSJT-X 是否正常解码 FT4：`yes / no`
- 固定频点 FT4 是否合格：`yes / no`
- 备注：

## 第二阶段：加入 patched 固件后的改频测试
- FT8 是否仍正常：`yes / no`
- FT4 是否仍进入 TX：`yes / no`
