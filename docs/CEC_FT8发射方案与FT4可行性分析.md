...
## 对周末探索分支的评价
周末那批尝试不是“完全没价值”，但它们更适合当**研究分支**，不适合接管当前主线。

### 1. `Data Mode + FT8 standby` 入口
相关提交：
- `f92a180 feat: Data Mode menu + FT8 standby via long press M`

这条线的价值在于：
- 证明可以在公开源码树里补一个更像样的人机入口
- 以后如果要做“完全自研固件”或“公开源码树分支实验”，这些 UI 入口设计可以继续复用

但它不解决当前主问题，因为：
- 当前项目主线已经确认必须以**真实 0.3q 二进制**为准
- 公开源码树不等于真实数字模式固件
- 所以它顶多是未来分支参考，不能当成现在的交付主线

### 2. `app/ft4_tx.c` 这类“直接在公开源码里做 FT4TX”
相关提交：
- `1e33eef feat: add FT4 TX as F1/F2 side function`
- `2013853 fix: radio accepts pre-encoded tones from WSJT-X via DigiManager`

这条线的思路是：
- 让主机侧直接送入 `0-3` 音调序列
- 电台侧只负责 `GFSK`/发射

这个方向本身有研究价值，但就当前实现看，还不够支撑“合格 FT4”：
- 它建立在公开源码树上，而不是建立在真实 `0.3q` 二进制上
- 当前主机桥接器把音调序列截成较短长度，不能代表完整标准 `FT4` 发射序列
- 发送定时使用的是比较粗的软时基，离“严格按 FT4 节拍发射”还有明显距离
- 它更像“证明能不能起发射”，而不是“证明发出来的是标准 FT4”

所以这条线的定位应该是：
- **未来完全自研发射链的实验分支**
- 不是当前最短成功路径

### 3. `tools/ft4dm` / `tools/ft4_digimanager.py` 这类自定义桥接器
这类工具最大的价值是：
- 快速验证“WSJT-X/样本 -> 电台”这条链能不能起发射
- 让我们更早看到“当前链路慢节拍”的现象

但它们不适合直接拿来当最终方案，原因也很清楚：
- 目前是实验性桥接器，不是作者真实 DigiManager 行为的严格复现
- 音调序列长度和发射节拍实现都偏验证性质
- 它们能证明“会发”，但不能证明“发的是合格 FT4”
...
- **可行，但更适合当研究分支**：
  - 公开源码树里自建 `FT4TX`
  - 自定义 `ft4dm` / Python 串口桥
- **当前最应该继续推进的主线**：
  - 真实 `0.3q` 固件继续作为无线机端基础
  - `patched DigiManager` 继续作为上位机端主改动点
  - 优先把 `FT4` 修成**固定频点下可被第二接收机 / WSJT-X 解码的合格信号**
  - 然后再把改频接回去

一句话说：

- 周末分支证明了“能发”这件事有探索价值
- 但当前主线必须盯住“能不能发出合格 FT4”
- 这也是为什么本项目当前优先级应该放在 **DigiManager 的 FT4 发送时序** 上，而不是先继续扩公开源码树里的自研发射器
